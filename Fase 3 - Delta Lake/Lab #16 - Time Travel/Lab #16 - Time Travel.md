...
A parte de Time Travel e retenção está alinhada com a documentação atual da Databricks: versões podem ser consultadas por versão ou timestamp; `DESCRIBE HISTORY` retorna as operações em ordem inversa; e Time Travel depende da retenção tanto do log quanto dos arquivos de dados. A documentação também informa os padrões de **30 dias para `delta.logRetentionDuration`** e **7 dias para `delta.deletedFileRetentionDuration`**.
```
...
A documentação explica que operações de atualização normalmente deixam arquivos antigos disponíveis justamente para permitir Time Travel; esses arquivos só são fisicamente removidos quando o VACUUM os elimina após a retenção aplicável.

https://docs.databricks.com/aws/en/sql/language-manual/delta-vacuum?utm_source=chatgpt.com

O padrão documentado atualmente para delta.deletedFileRetentionDuration é 7 dias. Depois que VACUUM remove os arquivos necessários para determinada versão histórica, tu perde a capacidade de fazer Time Travel para aquela versão.

E há outra retenção diferente:

`delta.logRetentionDuration` -> default: 30 dias

`delta.deletedFileRetentionDuration` -> default: 7 dias


Uma controla o histórico do log; a outra, quanto tempo arquivos de dados removidos permanecem elegíveis antes de serem eliminados pelo VACUUM. Para Time Travel funcionar, log + arquivos necessários precisam ainda estar disponíveis.

https://docs.databricks.com/aws/en/tables/history?utm_source=chatgpt.com