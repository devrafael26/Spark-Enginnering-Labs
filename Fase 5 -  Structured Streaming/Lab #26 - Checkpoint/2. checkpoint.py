...
# MAGIC %sql
# MAGIC CREATE VOLUME IF NOT EXISTS workspace.default.lab26_checkpoint;
...
# MAGIC %md
# MAGIC ### Experimento A — Primeira execução com checkpoint
# MAGIC - A ideia aqui é criar 30 registros, divididos em 3 arquivos, e executar a primeira leitura streaming.
...
# MAGIC %md
# MAGIC ### Experimento B — Inspecionar o checkpoint
# MAGIC - Agora não alteramos nada.
...
# MAGIC %md
# MAGIC #### O Experimento B não altera a streaming query. Ele apenas inspeciona o checkpoint produzido pelo Experimento A para observar como o Spark persistiu informações de progresso e identificação da query.

# COMMAND ----------

# MAGIC %md
# MAGIC ### Experimento C — Novos dados + mesmo checkpoint
# MAGIC
# MAGIC - Vamos adicionar mais 20 registros, sem apagar os 30 anteriores.
...
# MAGIC %md
# MAGIC #### OBS do Experimento C: 
# MAGIC - Reiniciar a stream significa iniciar uma nova execução da query; não significa zerar o checkpoint. Usando o mesmo checkpoint, o Spark recupera o progresso persistido anteriormente.

# COMMAND ----------

# MAGIC %md
# MAGIC ### Experimento D — Novo checkpoint
# MAGIC
# MAGIC - Agora mantemos exatamente os mesmos 50 registros no source, mas damos ao Spark um checkpoint vazio.
# MAGIC - Para não misturar os resultados, vamos usar também outra tabela destino.
...
# MAGIC %md
# MAGIC #### OBS Experimento D:
# MAGIC - O checkpoint não contém os dados da source. Ele contém o progresso da query. Ao usar um checkpoint novo, o Spark perde o conhecimento do que já havia sido processado e passa a tratar os dados ainda disponíveis na fonte como não processados para aquela nova query.

# COMMAND ----------

# MAGIC %md
# MAGIC ### Experimento E — id vs runId
# MAGIC - Aqui o objetivo é capturar as informações da query enquanto temos acesso ao objeto StreamingQuery.
...
# MAGIC %md
# MAGIC #### OBS Experimento E:
# MAGIC - Reutilizar o mesmo checkpoint preserva a identidade lógica da streaming query (query.id), enquanto cada nova execução possui um identificador próprio (runId).

# COMMAND ----------

# MAGIC %md
# MAGIC ### O fluxo completo do laboratório
