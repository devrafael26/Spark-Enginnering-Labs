...
## Conclusão
Ao observar o plano físico na saída do `df_result5m.explain(True)`, podemos observar que a agregação por `groupBy()` exige redistribuição dos dados entre as partições, pois registros pertencentes à mesma chave de agrupamento podem estar distribuídos em partições diferentes e precisam ser reorganizados para que possam ser processados juntos. Essa redistribuição é caracterizada pela presença de operadores como `Shuffle Exchange`, `Shuffle Map Stage` e `Hash Partitioning`.
